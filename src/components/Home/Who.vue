<template>
  <section class="flex items-center justify-center px-[40px] md:px-[70px] xl:px-[112px] py-[70px] lg:py-[88px] flex-col lg:flex-row">
    <div class="max-w-[542px] w-full mb-10 lg:mb-0 ">
      <img src="@/assets/images/Home/who.png" alt="" class="" />
    </div>
    <div class="lg:max-w-[618px] lg:ml-[80px] xl:ml-[120px] ">
      <span class="text-[18px] leading-[24px] font-medium text-id-yellow"
        >Who?</span
      >
      <h1
        class="text-[42px] text-id-dark-1 leading-[59px] tracking-[-0.02em] mt-[16px] font-millik"
      >
        Facial authentication
      </h1>
      <p class="text-[20px] leading-[30px] mt-[16px]">
        Compare customers’ facial biometrics with their government-issued
        identity document. Ensure that the person in front of the camera is who
        they claim to be.
      </p>
    </div>
  </section>
</template>
