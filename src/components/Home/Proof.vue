<template>
  <section
    class="flex items-center justify-center px-[40px] md:px-[70px] xl:px-[112px] py-[60px] lg:py-[88px] flex-col lg:flex-row"
  >
    <div class="lg:max-w-[618px] lg:mr-[80px] xl:mr-[160px] order-2 lg:order-1">
      <span class="text-[18px] leading-[24px] font-medium text-id-yellow"
        >Proof of address</span
      >
      <h1
        class="text-[28px] sm:text-[42px] text-id-dark-1 leading-[38px] sm:leading-[59px] tracking-[-0.02em] mt-[16px] font-millik"
      >
        Improve the accuracy of customers’ address
      </h1>
      <p class="text-[20px] leading-[30px] mt-[16px]">
        Check the validity of customers’ address with a picture of the address
        building and supporting documents as proof that the geoinformation
        provided is authentic.
      </p>
    </div>
    <div class="max-w-[562px] w-full order-1 lg:order-2 mb-10 lg:mb-0 ">
      <img src="@/assets/images/Home/proof.png" alt="" class="" />
    </div>
  </section>
</template>
